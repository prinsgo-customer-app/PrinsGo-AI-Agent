...
export const HermesScreen = () => {
  ...
  const fetchStatus = async () => {
    ...
    try {
      setError('');
      const res = await api.get(`/api/ai-agent/workspaces/${workspaceId}/hermes/status`);
      if (res.data?.success) {
        setStatus(res.data.data);
      } else {
        setError('Failed to fetch Hermes status');
      }
    } catch (err: any) {
      setError(err?.response?.data?.message || 'Hermes not configured or unavailable');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    >
      <View style={styles.content}>
        {loading ? (
          ...
          <View style={styles.card}>
            <Text style={styles.title}>Hermes Runtime Status</Text>
            <Text style={[styles.value, { color: status?.status === 'CONNECTED' ? '#10b981' : '#f59e0b' }]}>
              Status: {status?.status || 'UNKNOWN'}
            </Text>
            {status?.message && <Text style={styles.subtitle}>{status.message}</Text>}
            <Text style={styles.info}>Note: Hermes API keys and runtime configuration must be managed securely on the backend server.</Text>
          </View>
        )}
      </View>
    </ScrollView>
